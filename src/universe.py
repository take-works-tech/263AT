...
class Thresholds:
    ...
    #   $0.10-0.50  0.62  2.51  13.8%  7.8%  44.8%
    #   $0.50-1.00  0.86  5.69  14.4%  6.9%  30.1%
    #   **$1-2**    1.91  4.50  21.0%  9.8%  19.3%   ← すべての指標で最良
    #   $2-5        1.40  2.62  10.9%  3.4%  15.9%
    #   $20+        1.14  1.35   1.2%  0.10% 19.0%
    #
    # **$1-2 は $1未満より10倍株の出現率も高く、中央値も高く、半減率も低い。**
    # 一度「除外側の平均が 4.06 で高い」と見て下げようとしたが、
    # それは $0.50-1.00 の平均 5.69 が引き上げていただけで、
    # **中央値は 0.86（典型的に22%の損）だった。** 平均だけ見ると誤る。
    # → **$1 は据え置く。**
    #
    # 同時に分かったこと: **裾は $1-5 に集中している**のに、
    # ユニバースの 67% は $20以上で、そこの10倍株率は **0.10%**（$1-2 の98分の1）。
    # **裾が無い場所で裾を探している。**
    # これはゲートの問題ではなく、**重みの目的関数が平均だから**である。

...
| verified | **20**（★★★ の全件） |
| draft | 749 |
| ゲート方針 | **26/26 記述済み**（閾値は Phase 1 で較正） |

残っている問い（`validate_registry.py` が毎回出力する）:

| 問 | 残り | 備考 |
|---|---|---|
| q06 欠損の偏り | 749 | 全件で個別判断が要る |
| q09 相関 | 749 | データを見ないと答えられない。Phase 1 の相関行列（Z12）待ち |
| q10 経済的理由 | 749 | **人が書くしかない。ここが最も重要** |
| q11 実証度と一次文献 | **581** | **OSAP との突合で 188件を自動充填済み**（tools/crosswalk_oap.py）。残りは日本固有・LLM派生など OSAP に対応がないもの |
| q12 日米差 | 691 | |
| q05 分母ゼロ | 277 | 除算を含むもの |
| q01 会計期間 | 272 | A〜F・U・V |
| q04 会計基準差 | 272 | 同上 |
| q08 ∩の最適点 | 55 | |

### 推奨するレビュー順

1. **ゲート 26件の完全検証** — 誤ると致命的な損失に直結する
2. ★★★ 20件 — **完了**

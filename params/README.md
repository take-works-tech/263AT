...
661 パラメータの**機械可読な定義**。売買コードはこのレジストリから生成する。
...
```
  ...
  params/_defaults.yaml             … 2. プロジェクト規約（決定事項）
            ...
            │                            PITラグ / 会計期間の既定
            ▼
  params/_overrides.yaml            … 3. 人によるレビュー結果【最優先】
...
| verified | **20**（★★★ の全件） |
| draft | 641 |
| ゲート方針 | **25/25 記述済み**（閾値は Phase 1 で較正） |

残っている問い（`validate_registry.py` が毎回出力する）:

| 問 | 残り | 備考 |
|---|---|---|
| q06 欠損の偏り | 641 | 全件で個別判断が要る |
| q09 相関 | 641 | データを見ないと答えられない。Phase 1 の相関行列（Z12）待ち |
| q10 経済的理由 | 641 | **人が書くしかない。ここが最も重要** |
| q11 実証度と一次文献 | 641 | Open Source Asset Pricing の収録有無が判定の出発点になる |
| q12 日米差 | 591 | |
| q05 分母ゼロ | 242 | 除算を含むもの |
| q01 会計期間 | 225 | A〜F・U・V |
| q04 会計基準差 | 225 | 同上 |
| q08 ∩の最適点 | 55 | |

### 推奨するレビュー順

1. **ゲート 25件の完全検証** — 誤ると致命的な損失に直結する
2. ★★★ 20件 — **完了**
3. `_defaults.yaml` の初期重み上位カテゴリ（B / A / C / O）の ★★
4. 残り
